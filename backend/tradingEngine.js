class TradingEngine {
  ...
  getStatus() {
    return {
      ...
      totalCandlesProcessed: this.totalCandlesProcessed,
  ...
  getCandleBoundary(timestamp = Date.now()) {
    const duration = this.timeframeMilliseconds();
  ...
  analyzeMarket(data = {}) {
    ...
    if (
      ...
      return {
        signal: "WAIT",
        confidence: 0,
        reason: "Invalid candle close data"
      };
    }
    ...
    if (
      ...
      confidence = Math.min(
        95,
        60 +
          (this.settings.rsiSellLevel - rsi) +
          10 +
          10 +
          10
      );
  ...
  reset() {
    ...
    return {
      ok: true,
      message: "AI MONSTER U engine reset"
    };
  }
}
